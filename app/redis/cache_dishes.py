...
class DishesService:
    ...
    async def get_list_dishes(self, id_menu: int, id_submenu: int) -> list[DishesResponse]:
        cached_data = redis_client.get('/api/v1/menus/' + str(id_menu) + '/submenus/' + str(id_submenu) + '/dishes')
        if cached_data:
    ...
    async def get_one_dish_by_id(self, id_menu: int, id_submenu: int, id_dishes: int) -> DishesResponse:
        ...
        repo_d = RepositoriesDishes(self.db)
        res = await repo_d.get(id_dishes)
        if res:
            redis_client.setex(
    ...
    async def update_dish(
        self, id_menu: int, id_submenu: int, id_dishes: int, data: CreateDishesRequest
    ) -> DishesResponse:
        repo_d = RepositoriesDishes(self.db)
        res = await repo_d.update(id_dishes, data)
        response_dish = DishesResponse(id=res.id, title=res.title, description=res.description, price=res.price)
        redis_client.setex(
    ...
    async def create_dish(self, data: CreateDishesRequest, id_menu: int, id_submenu: int) -> DishesResponse | None:
        repo_d = RepositoriesDishes(self.db)
        res = await repo_d.create(data, id_submenu)
        if res:
